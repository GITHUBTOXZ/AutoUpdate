...
namespace AutoUpdate.Utils
{
    ...
    {
        //基本设置
       public static  string ftpip = @"ftp://192.168.1.113/";//ftp地址
       public static  string username = "admin";   //ftp用户名
       public static  string password = "111111";   //ftp密码
        ...
        public static string[] GetFileList(string dir)
        {
            ...
            try
            {
                request = (FtpWebRequest)FtpWebRequest.Create(new Uri(ftpip));
        ...
        public static int GetFileSize(string file)
        {
            ...
            try
            {
                request = (FtpWebRequest)FtpWebRequest.Create(new Uri(ftpip + file));
                request.UseBinary = true;
                request.Credentials = new NetworkCredential(username, password);//设置用户名和密码
        ...
        public static List<string> GetDirctory(string RequedstPath)
        {
            ...
            try
            {
                string uri = ftpip + RequedstPath;   //目标路径 path为服务器地址
                FtpWebRequest reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri(uri));
                // ftp用户名和密码
                reqFTP.Credentials = new NetworkCredential(username, password);
        ...
        /// <param name="saveFilePath">下载的文件保存的路径</param>
        public static void Download(string fileName,string saveFilePath)
        {
            try
            {
                ...
                reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri(ftpip + fileName));
